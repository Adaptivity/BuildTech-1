...
public class TileIronTank extends TileBuildCraft implements IFluidHandler {

	public final Tank tank = new Tank("tank", FluidContainerRegistry.BUCKET_VOLUME * 20, this);
	 ...
	 */
	public TileIronTank getBottomTank() {

		TileIronTank lastTank = this;

		while (true) {
			TileIronTank below = getTankBelow(lastTank);
			if (below != null) {
				lastTank = below;
			} else {
				break;
	...
	public TileIronTank getTopTank() {

		TileIronTank lastTank = this;

		while (true) {
			TileIronTank above = getTankAbove(lastTank);
			if (above != null) {
				lastTank = above;
			} else {
				break;
	...
	public static TileIronTank getTankBelow(TileIronTank tile) {
		TileEntity below = tile.getWorldObj().getTileEntity(tile.xCoord, tile.yCoord - 1, tile.zCoord);
		if (below instanceof TileIronTank) {
			return (TileIronTank) below;
		} else {
			return null;
		}
	}

	public static TileIronTank getTankAbove(TileIronTank tile) {
		TileEntity above = tile.getWorldObj().getTileEntity(tile.xCoord, tile.yCoord + 1, tile.zCoord);
		if (above instanceof TileIronTank) {
			return (TileIronTank) above;
		} else {
			return null;
		}
	}
	...
	public void moveFluidBelow() {
		TileIronTank below = getTankBelow(this);
		if (below == null) {
			return;
		}
	...
	public int fill(ForgeDirection from, FluidStack resource, boolean doFill) {
		...
		int totalUsed = 0;
		TileIronTank tankToFill = getBottomTank();
	...
	public FluidStack drain(ForgeDirection from, int maxEmpty, boolean doDrain) {
		TileIronTank bottom = getBottomTank();
	...
	public FluidTankInfo[] getTankInfo(ForgeDirection direction) {
		...
		TileIronTank tile = getBottomTank();
