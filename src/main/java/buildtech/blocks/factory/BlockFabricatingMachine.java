...
public class BlockFabricatingMachine extends BlockContainer 
{
	IIcon textureFront;
	IIcon textureTop;
	IIcon textureSide;
	...
	public BlockFabricatingMachine(int x,Block block) 
	{
		super(Material.iron);
	...
	{
		return new TileFabricatingMachine();
	}

	@Override
	public void breakBlock(World world, int x, int y, int z, Block block, int meta) 
	{
		TileFabricatingMachine fabricator = (TileFabricatingMachine) world.getTileEntity(x, y, z);
		fabricator.openInventory();
	        for (int t = 0; t < 2; t++){
	            ItemStack stack = fabricator.getStackInSlot(t);
	            if (stack != null) {
	            	fabricator.setInventorySlotContents(t, null);
	                BuildTechUtilities.dropItemstack(world, x, y, z, stack);
	            }
	        }
	        fabricator.closeInventory();
